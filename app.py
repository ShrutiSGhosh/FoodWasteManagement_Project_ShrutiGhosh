...
# CONFIG / PATHS
# ---------------------------
# Relative local data path (works if repo is cloned)
LOCAL_DATA_PATH = os.path.join(os.path.dirname(__file__), "..", "data")
LOCAL_DATA_PATH = os.path.abspath(LOCAL_DATA_PATH)

# GitHub raw base (works when evaluator pulls and runs anywhere)
# Repo: ShrutiSGhosh/FoodWasteManagement_Project_ShrutiGhosh
# Branch: main
RAW_BASE = (
    "https://raw.githubusercontent.com/"
    "ShrutiSGhosh/FoodWasteManagement_Project_ShrutiGhosh/main/FoodWasteApp/data"
)
...
# ---------------------------
# LOADERS (GitHub-first, local fallback)
# ---------------------------
def gh_url(fname: str) -> str:
    return f"{RAW_BASE}/{fname}"

@st.cache_data(show_spinner=False)
def load_csv_safely(fname: str) -> pd.DataFrame:
    """
    Try GitHub raw first, then local ./data. Returns empty DataFrame if both fail.
    """
    # 1) GitHub raw
    try:
        df = pd.read_csv(gh_url(fname))
    ...
    # 2) Local fallback
    local_path = os.path.join(LOCAL_DATA_PATH, fname)
    try:
...
def logo_bytes() -> bytes | None:
    ...
    import requests
    try:
        r = requests.get(gh_url(LOGO_FILE), timeout=5)
        if r.ok:
            return r.content
    except Exception:
        pass
    # Local fallback
    local_logo = os.path.join(LOCAL_DATA_PATH, LOGO_FILE)
...
# Load all CSVs
dfs = {k: load_csv_safely(v) for k, v in FILES.items()}
...
if page == "Overview":
    ...
    if not dfs["q05"].empty:
        ...
        if qty_cols:
            try:
                total_q = int(pd.to_numeric(df05[qty_cols[0]], errors="coerce").fillna(0).sum())
            except Exception:
...
# ---------------------------
    ...
    st.write("Each query below shows title + business question. You can edit the table in-session (not saved to disk). Use global filters to narrow data.")
...
# ---------------------------
    ...
    **What this app provides**
    - Loads 15 cleaned query CSVs from GitHub raw (with local fallback).
    ...
    - All queries are editable in **demo-only** mode using `st.data_editor` — changes remain only for the session.
    - Download edited previews (session CSV) if you want to export session edits locally.
    """)
    st.markdown("**Remote data source (GitHub Raw):**")
    st.code(RAW_BASE)
    st.markdown("**Local fallback path:**")
    st.code(LOCAL_DATA_PATH)
    st.markdown("**Files:**")
